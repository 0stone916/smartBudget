...
**400만 건 규모의 지출 내역 조회 성능 최적화**

* **[Problem] 시스템 물리적 한계**
    * 조회 요청 시 23초의 응답 지연 및 서버 메모리 부하 발생.
    * `OFFSET` 페이징으로 인한 I/O 비용 증가 및 `ORDER BY` 절의 `Filesort`로 인한 리소스 부하.
    

* **[Solution] 인덱스 및 쿼리 튜닝**
    * **복합 인덱스 최적화**: `(user_id, year, month, day DESC, id DESC)` 단일 복합 인덱스를 설계하여 검색 경로 최적화 및 `Filesort` 원천 방지.
    
    * **No-Offset 페이징 도입**: 마지막 조회 식별자(`day`, `id`)를 활용한 커서 기반 페이징으로 데이터 규모와 관계없는 일관된 응답 속도 확보.
    * **옵티마이저 통제**: `EXPLAIN` 분석을 통해 실행 계획을 최적화하여 조회 시간을 16ms로 단축 (1,000배 이상 성능 개선).

* **[Learning]**
    * 데이터베이스 엔진의 동작 원리(옵티마이저, 인덱스 트리 구조)를 이해하고 통제하는 것이 백엔드 엔지니어의 핵심 역량임을 체감함.
